...
export class HomePage implements OnInit {
  ...
  async initStorage() {
    this.storage = await this.storageService.create(); // Inisialisasi storage
  }
  ...
  async fetchCustomerData() {
    try {
      ...
      // Simpan data customer ke dalam storage
  ...
  async checkResi() {
    ...
    this.storage?.get(`resiData_${this.resiNumber}`).then((data: any) => {
      if (data) {
        this.resiData = data;
  ...
  fetchResiData() {
    ...
    this.http.get<any>(url, { headers }).subscribe(
      (response) => {
        ...
        // Simpan data resi ke dalam storage
